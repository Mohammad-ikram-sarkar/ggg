    ...
    <div>
      {filteredData.length > 0 ? (
        filteredData.map((booking) => (
          <div key={booking.id} className="mx-[160px] my-[20px] border-gray-200 border-2 py-[50px] rounded-2xl">
            <div className="flex justify-between items-center mx-[32px]">
              <div>
                <h1 className="text-[20px] font-bold text-[#141414]">{booking.name}</h1>
                <h1 className="text-[#14141499] font-medium">{booking.specialization}</h1>
              </div>
              <h1 className="text-[#14141499] font-medium">Appointment Fee: {booking.fee} Taka</h1>
            </div>
            <div className="flex justify-center items-center mx-[32px] text-[#FF0000] rounded-[99px] mt-[30px] border-2">
              <button className="font-medium" onClick={() => handleCancel(booking.id)}>
                Cancel Appointment
              </button>
            </div>
          </div>
